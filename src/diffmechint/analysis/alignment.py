"""Cross-tokenizer dictionary validation: probe transfer, residual geometry, activation proxy (E14-E18, E27)."""
...
def cell_path(root: Path, condition: str, dit_step: int, layer: int, t_bin: int) -> Path:
    return root / condition / f"step_{dit_step}" / f"{layer}_{t_bin}.h5"


def load_manifest(root: Path, condition: str, dit_step: int) -> dict:
...
def resolve_labels(latents_root: Path, condition: str, sample_idx: np.ndarray) -> np.ndarray:
...
def aligned_positions(
    ...
    left_idx = np.asarray(load_manifest(root, left_condition, dit_step)["sample_idx"], dtype=np.int64)
    right_idx = np.asarray(load_manifest(root, right_condition, dit_step)["sample_idx"], dtype=np.int64)
...
def read_rows(path: Path, rows: np.ndarray) -> np.ndarray:
...
def run_probe_transfer(args: argparse.Namespace) -> int:
    ...
    for layer, t_bin in _selected_cells(args):
        ...
        for source, target in permutations(args.conditions, 2):
            ...
            src_pos, tgt_pos, sample_idx = aligned_positions(
                args.activations_root, source, target, args.dit_step, args.max_images, args.seed
            )
            src_path = cell_path(args.activations_root, source, args.dit_step, layer, t_bin)
            tgt_path = cell_path(args.activations_root, target, args.dit_step, layer, t_bin)
            src_acts = read_rows(src_path, src_pos)
            tgt_acts = read_rows(tgt_path, tgt_pos)
            labels = resolve_labels(args.latents_root, source, sample_idx)
...
def run_residual_similarity(args: argparse.Namespace) -> int:
    ...
    def add_pair(
        ...
        left_pos, right_pos, sample_idx = aligned_positions(
        ...
        left = read_rows(
            cell_path(args.activations_root, left_cond, args.dit_step, left_layer, left_t),
            left_pos,
        )
        right = read_rows(
            cell_path(args.activations_root, right_cond, args.dit_step, right_layer, right_t),
            right_pos,
        )
        labels = resolve_labels(args.latents_root, left_cond, sample_idx)
...
def run_activation_proxy(args: argparse.Namespace) -> int:
    ...
    for layer, t_bin in _selected_cells(args):
        for source, target in permutations(args.conditions, 2):
            ...
            src_acts = read_rows(
                cell_path(args.activations_root, source, args.dit_step, layer, t_bin),
                src_pos,
            )
            tgt_acts = read_rows(
                cell_path(args.activations_root, target, args.dit_step, layer, t_bin),
                tgt_pos,
            )
